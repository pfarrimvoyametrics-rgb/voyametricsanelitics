...
function Badge({ children, cor }) {
  return <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${cor}`}>{children}</span>;
}

/** Gestão de utilizadores: criar operadores/admins, ativar/desativar, redefinir senha. */
                ...
                <div key={u.id} className="flex items-center justify-between gap-3 px-5 py-3">
                  <div className="min-w-0">
                    <p className="truncate text-sm font-medium text-slate-800">
                      {u.nome} {!u.ativo && <Badge cor="bg-slate-100 text-slate-500">inativo</Badge>}
                    </p>
                    <p className="truncate text-xs text-slate-500">{u.email}</p>
                  </div>
                  <div className="flex shrink-0 items-center gap-2">
                    <Badge cor={u.funcao === 'super_admin' ? 'bg-violet-100 text-violet-700'
                      : u.funcao === 'admin' ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-600'}>
                      {ROTULO_FUNCAO[u.funcao] || u.funcao}
                    </Badge>
                    {u.categoria && <Badge cor="bg-amber-100 text-amber-700">{ROTULO_CATEGORIA[u.categoria] || u.categoria}</Badge>}
                    {u.funcao !== 'super_admin' && (
                      <>
                        <button onClick={() => redefinirSenha(u)} title="Redefinir palavra-passe"
                          className="rounded-md border border-slate-300 px-2 py-1 text-xs text-slate-600 transition hover:bg-slate-50">Senha</button>
