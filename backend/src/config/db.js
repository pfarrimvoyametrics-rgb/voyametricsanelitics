...
/** Aplica o seed SQL (dados não sensíveis, ex.: regras de triagem). */
async function semearSql() {
  const ficheiro = path.join(__dirname, '..', 'db', 'seeds', 'seed.sql');
  console.log('[db] A aplicar seed SQL (regras de triagem)…');
  await executarFicheiroSql(ficheiro);
}

module.exports = { pool, migrar, semearSql, executarFicheiroSql };
