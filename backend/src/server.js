...
async function iniciar() {
  ...
  // Garante o super administrador do servidor (credenciais via ambiente).
  try {
    const userService = require('./services/userService');
    const sa = await userService.garantirSuperAdmin();
    if (sa.criado) {
      console.log(`[auth] Super admin pronto: ${sa.utilizador.email}`);
    } else {
      console.warn(
        '[auth] Super admin não configurado — defina SUPER_ADMIN_EMAIL e ' +
          'SUPER_ADMIN_PASSWORD (ou corra `npm run create-admin`).'
      );
    }
  } catch (err) {
    console.error('[auth] Falha ao garantir o super admin:', err.message);
  }
