...
router.get('/', exigirAutenticacao, async (req, res) => {
  ...
  const lista = await ticketModel.listarParaUtilizador(req.utilizador, {
    status,
    categoria,
...
router.get('/metricas', exigirAutenticacao, exigirAdmin, async (_req, res) => {
  const [carga, sla] = await Promise.all([
    ticketModel.cargaPorOperador(),
    ticketModel.metricasSla(),
  ]);
  res.json({ carga, sla });
});

// POST /api/tickets/:id/assumir
router.post('/:id/assumir', exigirAutenticacao, async (req, res) => {
  const io = req.app.get('io');
  const r = await ticketService.assumir(io, req.params.id, req.utilizador);
  if (!r.ok) return res.status(409).json({ erro: r.motivo });
  res.json(r.ticket);
});

// POST /api/tickets/:id/libertar
router.post('/:id/libertar', exigirAutenticacao, async (req, res) => {
  const io = req.app.get('io');
  // Admin pode libertar qualquer; operador só os seus.
  const opts =
    req.utilizador.funcao === 'admin'
      ? { porSistema: true }
      : { operadorId: req.utilizador.id };
...
router.post('/:id/resolver', exigirAutenticacao, async (req, res) => {
  const io = req.app.get('io');
  const { resposta } = req.body || {};

  const ticket = await ticketModel.porId(req.params.id);
  ...
  const r = await ticketService.resolver(io, req.params.id, req.utilizador.id);
  if (!r.ok) return res.status(409).json({ erro: r.motivo });
  res.json(r.ticket);
});
